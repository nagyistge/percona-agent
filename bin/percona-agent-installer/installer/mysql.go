...
func MakeGrant(dsn mysql.DSN, user string, pass string) []string {
	...
	grants := []string{
		fmt.Sprintf("GRANT SUPER, PROCESS, USAGE, SELECT ON *.* TO '%s'@'%s' IDENTIFIED BY '%s' WITH MAX_USER_CONNECTIONS ?", user, host, pass, mysql.MAX_OPEN_CONNECTIONS),
		fmt.Sprintf("GRANT UPDATE, DELETE, DROP ON performance_schema.* TO '%s'@'%s' IDENTIFIED BY '%s' WITH MAX_USER_CONNECTIONS ?", user, host, pass, mysql.MAX_OPEN_CONNECTIONS),
	}
	return grants
}
